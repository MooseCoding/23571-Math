...
java_lines.append("    }")
java_lines.append("")
java_lines.append("    // Get interpolated values for a (possibly fractional) index 'distance'.")
java_lines.append("    // If out of bounds, clamps to valid range and returns the endpoint.")
java_lines.append("    public static double[] getValues(double distance) {")
java_lines.append("        if (Double.isNaN(distance)) {")
java_lines.append("            return new double[] { Double.NaN, Double.NaN };")
java_lines.append("        }")
java_lines.append("        int low = (int) Math.floor(distance);")
java_lines.append("        int high = (int) Math.ceil(distance);")
java_lines.append("        int n = points.length;")
java_lines.append("        if (n == 0) {")
java_lines.append("            return new double[] { Double.NaN, Double.NaN };")
java_lines.append("        }")
java_lines.append("        // clamp indices to [0, n-1]")
java_lines.append("        if (low < 0) low = 0;")
java_lines.append("        if (high < 0) high = 0;")
java_lines.append("        if (low >= n) low = n - 1;")
java_lines.append("        if (high >= n) high = n - 1;")
java_lines.append("        if (high == low) {")
java_lines.append("            // exact index or both clamped to same endpoint")
java_lines.append("            return new double[] { points[low][0], points[low][1] };")
java_lines.append("        }")
java_lines.append("        double t = distance - low;")
